...
from pharmacy.pdf_generator import generate_pdf
...
def pdf_view(request):
    ...
    response['Content-Disposition'] = 'filename="Billing_Report.pdf"'
...
class BillingCreateView(CreateView):
    ...
    template_name = 'create_billing.html'
    success_url = '/receptionist/billing/'

    def form_valid(self, form):
        billing = form.save(commit=False)
        # Calculate the total
        quantity_list = form.cleaned_data['quantity'].split(',')
        price_list = form.cleaned_data['price'].split(',')
        total = sum(int(qty) * int(price) for qty, price in zip(quantity_list, price_list))
        billing.total = str(total)
        billing.save()
...
class BillingListView(ListView):
    ...
    def get_queryset(self):
        ...
        # Loop through all the billings and calculate the total
        for billing in queryset:
            # Split the strings and convert them to integers
            quantity_list = [int(qty) for qty in billing.quantity.split(',')]
            price_list = [int(price) for price in billing.price.split(',')]
            # Calculate the total for this billing
            total = sum([qty * price for qty, price in zip(quantity_list, price_list)])
            # Assign the total to the billing object
            billing.total = str(total)
            billing.save()
        return queryset
